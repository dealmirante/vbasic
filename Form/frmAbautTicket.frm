...
Begin VB.Form frmAbautTicket 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Label Label2 
         BackColor       =   &H80000018&
         BorderStyle     =   1  'Fixed Single
         Caption         =   " Estado de Actualización de ventas del Interior"
         BeginProperty Font 
            Name            =   "MS Serif"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmAbautTicket"
...
Dim fch(0 To 4) As String
Dim Sdep(0 To 4) As String
Dim loc(0 To 4) As String
...
For i = 0 To 4
'"UPDATE estadis.control_procesos SET "
    sql$ = " INSERT INTO  estadis.control_procesos " _
    & " VALUES ( " & func_ToDate(MskFch(i).FormattedText) _
    & ", 'ES','INT','" & Sdep(i) & "'" _
    & ", 'S', '" & loc(i) & "' ) "
...
sql = " SELECT max(FECHA) fch ,cod_sdep " _
& " FROM estadis.CONTROL_PROCESOS " _
& " WHERE cod_proceso = 'ES' " _
& " GROUP BY COD_SDEP "
...
If Aplicacion.ObtenerRsDAO(sql, RS) Then
    ...
    Do While Not RS.EOF
        Select Case RS!Cod_Sdep
            Case BARI
                fch(0) = Format$(RS!fch, FTOFECHA)
            Case CORD
                fch(1) = Format$(RS!fch, FTOFECHA)
            Case MDPL
                fch(2) = Format$(RS!fch, FTOFECHA)
            Case MEND
                fch(3) = Format$(RS!fch, FTOFECHA)
        End Select
        RS.MoveNext
    Loop
